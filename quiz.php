...
if (isset($kurs)) {
 ...
 if (!isset($_POST["save"])) {
  ...
  if ($frage <= $fragen) {
   ...
   if ($frage < $fragen) {
    echo '<form action="" method="post"><fieldset><legend><b>Fragen</b></legend>' .
     '<p class="zaehler">Frage ' . ($frage + 1) . ' von ' . $fragen . ' | <a href="teilen.php?fragenid=' . ($eintrag["fragenid"]) . ' " target="_blank">Frage Teilen</a></p>' .
